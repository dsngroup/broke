...
def parseLog(log_name):
    ...
    for line in f:
        if "current time:" in line:
            bp_status = line.split("Back pressure status: ")[1]\
                            .split(" ")[0] == 'True'
            current_capacity = int(line.split("current capacity: ")[1].split(" ")[0])
            current_time = int(line.split("current time: ")[1].split("\n")[0])
            ret.append([bp_status, current_capacity, current_time])
    if len(ret) == 0:
...
def pruneAndResetOffsetLog(logs):
    minTime = logs[0][1][0][2]
    for log in logs:
        if minTime < log[1][0][2]:
            minTime = log[1][0][2]
    maxTime = logs[0][1][-1][2]
    for log in logs:
        if maxTime > log[1][-1][2]:
            maxTime = log[1][-1][2]
    for log in logs:
        log[1] = [line for line in log[1] if line[2] >= minTime]
        log[1] = [line for line in log[1] if line[2] <= maxTime]
        # Reset time offset
        for line in log[1]:
            line[2] = line[2] - minTime 
...
if __name__ == "__main__":
    ...
    for _, dirs, _ in os.walk(root_dir):
		...
		file_directory = filepath.split("/")[-2]
                logs.append([file_directory, parseLog(filepath)])
    ...
    pl.title('Same RTT & Same Computation Capability')
    pl.xlabel('Time(ms)')
    pl.ylabel('Inbound Queue Size of the Consumer')
    pl.legend()
    pl.savefig('./results/' + log_filename.split(".")[0])
